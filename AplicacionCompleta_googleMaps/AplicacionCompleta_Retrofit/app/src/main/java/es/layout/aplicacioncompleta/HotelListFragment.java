...
import es.layout.aplicacioncompleta.model.retrofit.Coordinate;
...
public class HotelListFragment extends Fragment {
    ...
    private void configurarRecyclerView() {
        ...
        // Cuando el usuario pulse un hotel, abriremos el mapa con sus coordenadas.
        hotelAdapter.setOnHotelClickListener(new HotelAdapter.OnHotelClickListener() {
            @Override
            public void onHotelClick(Hotel hotel) {
                abrirMapaHotel(hotel);
            }
        });
    ...
    /**
     * Abre el Fragment de Google Maps con los datos del hotel seleccionado.
     *
     * @param hotel hotel pulsado por el usuario en el RecyclerView.
     */
    private void abrirMapaHotel(Hotel hotel) {
        if (hotel == null) {
            Toast.makeText(requireContext(), "No se ha podido abrir el hotel", Toast.LENGTH_SHORT).show();
            return;
        }

        Coordinate coordinate = hotel.getCoordinate();

        // Si el servicio no trae coordenadas, no podemos pintar el marker.
        if (coordinate == null) {
            Toast.makeText(requireContext(), "Este hotel no tiene coordenadas", Toast.LENGTH_SHORT).show();
            return;
        }

        Bundle bundle = new Bundle();
        bundle.putString(HotelMapFragment.ARG_HOTEL_NAME, hotel.getName());
        bundle.putDouble(HotelMapFragment.ARG_HOTEL_LAT, coordinate.getLat());
        bundle.putDouble(HotelMapFragment.ARG_HOTEL_LON, coordinate.getLon());

        // Navegamos usando el NavHost principal de HomeActivity.
        // Esto evita que la app se cierre si el Fragment está dentro de un ViewPager2.
        Navigation.findNavController(requireActivity(), R.id.nav_host_fragment_main)
                .navigate(R.id.action_homeTabsFragment_to_hotelMapFragment, bundle);
    }
